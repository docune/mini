...
<script type="text/javascript">
	$(function() {
		$(window).resize(function() {
			//덧글 표시 영역의 부모 요소에 대한 가로 넓이를 구한다.
			var w = $('.container').width();
			$('.fb-comments').find('span').css('width', w);
			$('.fb-comments').find('iframe').css('width', w);
		});
...
<body>
	...
	<div class="container">
		...
		<div class="alert alert-info" role="alert">
			<h3 style='margin: 0'>
				<%=document.getSubject()%>
				<br /> <small> <%=document.getWriterName()%> <a
					href='mailto:<%=document.getEmail()%>'> <i
						class='glyphicon glyphicon-envelope'></i></a> / 조회수 : <%=document.getHit()%>
				...
				if (fileList.size() > 0) {
					...
					if (list_style.equals("gallery")) {
						...
						for (int i=0; i<fileList.size(); i++){
							...
							// 파일이 저장된 경로 (폴더경로 + "/" + 파일명)
							String file_path = fileItem.getFileDir() + '/' 
		...
		%>
							<p class="text-center">
								<img src="download.jsp?file=<%=enc_file_path%>&orgin_name=<%=enc_orgin_name%>"
						...
						<%
							// 조회된 파일의 수 만큼 반복
									for (int i = 0; i < fileList.size(); i++) {
										...
										// Get 파라미터 전달을 위해서 URLEncoding 처리
										String enc_file_path = URLEncoder.encode(file_path,
												"UTF-8");
										String enc_orgin_name = URLEncoder.encode(orgin_name,
												"UTF-8");
						%> <a
						href="download.jsp?file=<%=enc_file_path%>&orgin_name=<%=enc_orgin_name%>"
 ...
 %>
					...
					<td>
						<%
							...
							if (nextDocument == null) {
								...
								String link = String.format(link_format, nextDocument.getId(),
										nextDocument.getSubject());
								out.print(link);
							}
						%>
					</td>
					...
					<td>
						<%
							...
							if (prevDocument == null) {
								...
								// read.jsp로 다음글의 일련번호를 전달하는 링크를 구성한다.
								String link_format = "<a href='read.jsp?document_id=%d'>%s</a>";
								String link = String.format(link_format, prevDocument.getId(),
										prevDocument.getSubject());
								out.print(link);
							}
						%>
					</td>
		...
		<div class="clearfix">
			<div class="pull-right">
				...
				<a href='edit.jsp?document_id=<%=document_id%>' class='btn btn-warning'>수정하기</a>
				<!-- 삭제 대상을 지정하기 위하여 게시물 일련번호를 전달한다. -->
				<a href='delete.jsp?document_id=<%=document_id%>' class='btn btn-danger'>삭제하기</a>
			</div>
		</div>
		
		<!-- 덧글 작성 폼 -->
		<hr />
		<form id="comment_form" role="form" method="post"
			action="ajax/add_comment.jsp">
			<!-- 글 번호 상태 유지 -->
			...
			<fieldset>
				...
				<br />
					<div class="form-group">
						<label for="writer_name" class="error"></label>
					</div>
					<div class="form-group">
						<label for="writer_pw" class="error"></label>
					</div>
					<div class="form-group">
		...
		<!-- 페이스북 덧글 표시하기 -->
		<div class="fb-comments" data-href="http://localhost/15-MySite/bbs/read.jsp?document_id<%=document_id%>" 
