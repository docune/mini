...
<%
	...
	for (int i = 0; i < fileList.size(); i++) {
		logger.debug("FileInfo[" + i + "]=> "
				+ fileList.get(i).toString());
	}

	/** 작성 내용을 Beans 객체로 구성 */
	// --> 게시물일련번호, 회원일련번호와 조회수는 0으로, 날짜는 null로 설정한다.
	// --> 로그인한 경우 회원일련번호는 세션에서 추출한 값으로 설정한다.
	BBSDocument document = new BBSDocument(0, bbs_type, member_id, writer_name,
			writer_pw, email, subject, content, ip_address, 0, null,
			null);
	...
	for (int i = 0; i < fileList.size(); i++) {
			...
			BBSFile file = new BBSFile(0, document_id, f.getFileDir(),
					...
					f.getContentType(), f.getOrginName(), null, null);

			int file_id = 0;
			try {
				//데이터 저장하기
				file_id = sqlSession.insert("BBSFileMapper.doInsert",
						file);
			} catch (Exception e) {
				// 에러가 난 경우 처리할 내용이 있다면 구현한다.
				logger.error(e.getMessage());
