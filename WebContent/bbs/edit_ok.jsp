...
<%
	...
	/** WebHeler를 사용한 Multipart 데이터 처리 */
	...
	if (is_mine.equals("N")) {
		...
		if (saved_document == null) {
			sqlSession.close();
			helper.redirect(null, "게시물 조회에 실패헀습니다.");
			return;
		}
		...
		for (int i=0; i<delete_list.length; i++) {
			...
			for (int j=0; j<savedFileList.size(); j++) {
				...
				if (item.getId() == delete_target_id) {
					...
					try {
						sqlSession.update("BBSFileMapper.doDeleteItem", 
								delete_target_id);
					} catch (Exception e) {
						logger.error(e.getMessage());
					}
	...
	for (int i=0; i<fileList.size(); i++) {
		...
		BBSFile file = new BBSFile(0, document_id, f.getFileDir(),
				f.getFieldName(), f.getFileSize(), f.getContentType(),
