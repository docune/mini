...
<%
	...
	/** 작성 내용을 Beans 객체로 구성 */
	// --> 게시물 일련번호와 비밀번호만 설정한다.
	BBSDocument document = new BBSDocument(document_id, bbs_type,
			member_id, null, pwd, null, null, null, null, 0, null, null);
	...
	try {
		...
		fileList = sqlSession.selectList("BBSFileMapper.getFileList",
				document_id);
	} catch (Exception e) {
		// 에러가 난 경우 처리할 내용이 있다면 구현한다.
		logger.error(e.getMessage());
	...
	try {
		// 특정 게시물에 덧글정보 삭제하기
		comment_count = sqlSession.delete(
				"BBSCommentMapper.doDeleteAll", document_id);
	} catch (Exception e) {
		// 에러가 난 경우 처리할 내용이 있다면 구현한다.
		logger.error(e.getMessage());
	}
	...
	try {
		// 특정 게시물에 속한 파일정보 삭제하기
		delete_count = sqlSession.delete("BBSDocumentMapper.doDelete",
				document_id);
	} catch (Exception e) {
		// 에러가 난 경우 처리할 내용이 있다면 구현한다.
		logger.error(e.getMessage());
	...
	if (fileList != null) {
		for (int i = 0; i < fileList.size(); i++) {
			...
			// 파일이 실제로 존재한다면 삭제 후 결과를 로그로 기록한다.
			if (file.exists()) {
				file.delete();
