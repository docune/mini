...
<%
	...
	try {
		// 특정 게시물 정보 조회하기
		documentList = sqlSession.selectList("BBSDocumentMapper.getGalleryList", document);
	} catch (Exception e) {
		// 에러가 난 경우 처리할 내용이 있다면 구현한다.
		logger.error(e.getMessage());
	} finally {
...
<%
	...
	if (documentList.size() > 0) {
		...
		for (int i=0; i<documentList.size(); i++){
			...
			// Calender 클래스를 사용해서 timestamp로 변환한다.
			try {
			...
			<div class="col-xs-6 col-sm-4 col-md-4 col-lg-3">
				<div class="thumbnail">
					<a href="read.jsp?document_id=<%=item.getId()%>">
						<img src="download.jsp?file=<%=item.getFile()%>&img_size=320"
							style="width:100%"/></a>
					<div class="caption">
...
%>			
		...
		<nav class='text-center'>
			<ul class='pagination'>
				...
				<li>
					<a href="list.jsp?page=<%=pageHelper.getPrevPage()%>&keyword=<%=keyword%>">
				...
				%>
				<li><a href="list.jsp?page=<%=i%>&keyword=<%=keyword%>"><%=i%>
				</a></li>
				<%
						}
					}
				...
				<li>
					<a href="list.jsp?page=<%=pageHelper.getNextPage()%>&keyword=<%=keyword%>">
