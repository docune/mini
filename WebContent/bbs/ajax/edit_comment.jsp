...
<%
	...
	/** 작성 내용을 Beans 객체로 구성 */
	// --> 덧글 일련번호와 비밀번호만 설정한다.
	BBSComment comment = new BBSComment(comment_id, 0, member_id, writer_name,
			writer_pw, email, content, ip_address, null, null);
	...
	if (is_mine.equals("N")) {
		...
		// 조회결과가 있다면?
		if (comment != null) {
			// 덧글에 저장된 회원번호와 현재 내 세션의 회원번호를 비교한다.
		...
		comment.setIpAddress(ip_address);
	}
